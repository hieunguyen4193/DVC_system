...
data_name = "MRD_GW_UMI_Phuc_HG19_release_20240606"
if os.path.isfile(os.path.join(path_to_save_prep_metadata, "{}.metadata.csv".format(data_name))) == False:
    all_files = [item for item in pathlib.Path("/Users/hieunguyen/src/DVC_system/examples/dummy_from_real/{}".format(data_name)).glob("*/*")
    ...
    metadata_mrd = pd.read_excel("/Users/hieunguyen/src/DVC_system/ECD_metadata/metadata_MRD_20240606.xlsx")[["SampleID", "Cancer", "True_label"]]
    ...
    maindf["Description"] = "feature generated from WGS and MRD data"
    maindf["FeatureVersion"] = "v0.1"
    maindf["Date"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    maindf["FeatureName"] = maindf["FileName"].apply(lambda x: "_".join(x.split("_")[2:]).split(".")[0])

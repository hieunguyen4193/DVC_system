...
all_files = [item for item in pathlib.Path("{}/examples/dummy_from_real/{}/cov".format(path_to_project_src, data_name)).glob("*")
...
if os.path.isfile(os.path.join(path_to_save_prep_metadata, "{}_cov.metadata.csv".format(data_name))) == False:
    ...
    maindf["Labcode"] = maindf["FileName"].apply(lambda x: 
        x.replace("pooled_", "").split("_")[0].split("-")[1] if "-" in x else x.replace("pooled_", "").split("_")[0])
    maindf["Labcode"] = maindf["Labcode"].apply(lambda x: x.replace(".deduplicated.bedGraph.gz.bismark.zero.cov", ""))
    maindf["SequencingID"] = maindf["Labcode"]
    maindf["Date"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    maindf["methylationCaller"] = "bismark_methylation_extractor"
    maindf["methylationCallerParams"] = "default"
    maindf["Project"] = "PBMC"
    ...
    maindf.to_csv(os.path.join(path_to_save_prep_metadata, "{}_cov.metadata.csv".format(data_name)), index = False)
else:
    maindf = pd.read_csv(os.path.join(path_to_save_prep_metadata, "{}_cov.metadata.csv".format(data_name)))

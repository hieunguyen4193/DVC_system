...
data_name = "MRD_WGS_hg19"

all_files = [item for item in pathlib.Path("{}/examples/dummy_from_real/{}/iwFAF".format(path_to_project_src, data_name)).glob("*")
...
if os.path.isfile(os.path.join(path_to_save_prep_metadata, "{}_iwFAF.metadata.csv".format(data_name))) == False:
    ...
    maindf["Description"] = "iwFAF feature, nucleosome feature"
    maindf["FeatureVersion"] = "v0.1"
    maindf["Date"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    maindf["FeatureName"] = "iwFAF"
    maindf["Pipeline"] = maindf["Labcode"].apply(lambda x: 
                                                        "WGS_hg19" if x in metadata[metadata["project"] == "WGS"]["SampleID"].values else "MRD_hg19")
    maindf["Pipeline_params"] = "default"
    maindf["Pipeline_repo"] = "gitlab"
    maindf["Feature_Pipeline"] = "ECD_WGS_MRD_feature_pipeline"
    maindf["Feature_Pipeline_params"] = "default"
    maindf["Feature_Pipeline_repo"] = "gitlab"
    maindf["Ref_genome"] = "hg19"
    maindf["Input"] = "bam alignment from WGS and MRD data"
    maindf["Input_type"] = "bam"
    ...
    maindf.to_csv(os.path.join(path_to_save_prep_metadata, "{}_iwFAF.metadata.csv".format(data_name)), index = False)
else:
    maindf = pd.read_csv(os.path.join(path_to_save_prep_metadata, "{}_iwFAF.metadata.csv".format(data_name)))
...
bamBucket = RDSBucket( 
                    ...
                    bucketName = "ecd-wgs-feature",
                    PROFILE_NAME = "wgsfeature",
